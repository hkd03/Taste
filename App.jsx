...
async function ai(prompt, fallback) {
  ...
  try {
    ...
    if (data.error) { console.error("API error:", data.error.message || data.error); return fallback; }
    if (!data.content) { console.error("No content in response:", data); return fallback; }
            ...
            <div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {(results.recommendations || []).map(function(rec, i) {
                  ...
                  return (
                    <RecCard key={i} rec={rec} cat={cat} index={i}
                      onAdd={function(payload) { addToLib(Object.assign({}, rec, payload)); }} />
                  );
                })}
              </div>
